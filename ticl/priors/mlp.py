...
class MLP(torch.nn.Module):
    def __init__(self, device, num_features, num_outputs, n_samples, sampling, *, num_layers, prior_mlp_hidden_dim, prior_mlp_activations,
        ...
        self.num_layers_effect = np.random.randint(2, 4)
        ...
        with torch.no_grad():
            ...
            def generate_mlp(layers_n, is_eff):

                layers = [nn.Linear(self.num_causes, self.prior_mlp_hidden_dim, device=device)]
                layers += [module for layer_idx in range(layers_n-1) for module in generate_module(layer_idx, self.prior_mlp_hidden_dim)]
                if not self.is_causal or is_eff:
                    layers += generate_module(-1, num_outputs)
                layers = nn.Sequential(*layers)

                # Initialize Model parameters
                for i, (n, p) in enumerate(layers.named_parameters()):
            ...
            self.layers_0 = generate_mlp(self.num_layers, False)
            self.layers_eff = generate_mlp(self.num_layers_effect, True)
...
class MLPPrior:
    ...
    def get_batch(self, batch_size, n_samples, num_features, device=default_device, num_outputs=1, epoch=None, single_eval_pos=None):
        ...
        y0 = torch.cat(y0, 1).detach()
        y1 = torch.cat(y1, 1).detach()
