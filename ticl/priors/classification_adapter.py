...
class ClassificationAdapter:
    ...
    def __call__(self, batch_size, n_samples, num_features, device, epoch=None, single_eval_pos=None):
        ...
        x, y_0, y_1 = self.base_prior.get_batch(**args)
        ...
        x = normalize_data(x)
        y_0, y_1 = self.normalize_cate(y_0, y_1)
        ...
        if torch.isnan(y_0).any():
            print('Nans in target y_0!')
            if self.h['max_num_classes'] == 0:
                y_0[torch.isnan(y_0)] = 0
            else:
                y_0[torch.isnan(y_0)] = -100
                
        if torch.isnan(y_1).any():
            print('Nans in target y_1!')
            if self.h['max_num_classes'] == 0:
                y_1[torch.isnan(y_1)] = 0
            else:
                y_1[torch.isnan(y_1)] = -100
        ...
        return x, y_0, y_1, info  # x.shape = (T,B,H)
...
class ClassificationAdapterPrior:
    ...
    def get_batch(self, batch_size, n_samples, num_features, device, epoch=None, single_eval_pos=None):
        with torch.no_grad():
            ...
            x, y_0, y_1, info = ClassificationAdapter(self.base_prior, self.config)(batch_size=batch_size, **args)
            x, y_0, y_1 = x.detach(), y_0.detach(), y_1.detach()

        return x, y_0, y_1, info
